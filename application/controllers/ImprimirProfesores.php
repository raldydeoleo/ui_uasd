<?php if (!defined('BASEPATH'))
     ...
     * Matricula: 94-3032
     * Controlador que maneja la impresion de lista de  profesores.
     * 30-07-2018
     */
class ImprimirProfesores extends CI_Controller {
    ...
    public function index()
    {
        ...
        $this->cezpdf->ezText('Listado de Profesores',15);  //insert text with size
       ...
       // $this->cezpdf->ezSetY(740);
        ...
        $data_table=array();
        foreach ($profesores->result_array() as $row) {
            $data_table[]=$row;
        }
...
public function pdf_nomina(){
    ...
    public function pdf_profesor($id_profesor)
    {
        error_reporting(0);  //suppress some error message
        $parameters=array(
            'paper'=>'Letter',   //paper size
        ...
        $this->cezpdf->ezText('Sistema de Planificacion de Asignaturas [SPA-UASD]',20);  //insert text with size
        $this->cezpdf->ezText('Detalle de Profesor',15);  //insert text with size
        $this->cezpdf->ezText('', 9);
        $this->cezpdf->ezText('', 9);
       ...
       // $this->cezpdf->ezSetY(740);
        ...
        $profesor=$this->db->query('SELECT * FROM profesores WHERE id_profesor LIKE ' .$id_profesor);
     ...
     $otras=$this->db->query('SELECT * FROM percepciones WHERE fecha_percepcion LIKE  "30-04-2017" AND id_empleado LIKE ' .$id_empleado);
        */
        ...
        $profesor_table=array();
        foreach ($profesor->result_array() as $row) {
            $profesor_table[]=$row;
        }
        ...
        $this->cezpdf->ezTable($profesor_table, $profesor_header,'Datos del Profesor',array('showLines'=>0 , )); //generate table
       ...
       // $this->cezpdf->ezText('Firma del empleado:_______________________________',10);
        ...
        //$this->cezpdf->ezText("<b>Fecha:</b> ".date("d/m/Y"), 10);
        //$this->cezpdf->ezText("<b>Hora:</b> ".date("H:i:s")."\n\n", 10);
        $this->cezpdf->ezStream(array('Content-Disposition'=>'detalle_profesor.pdf'));  //force user to download the file as 'just_random_filename.pdf'
    }
